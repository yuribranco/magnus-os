...
- **2026-06-12:** Abordagem decidida (Yuri, /ceo): **Haiku nas baixo-julgamento + medir no dogfood** (NÃO harness sintético — ganho de rigor marginal tendo dogfood real medindo). `panel.model: haiku` setado em **5 skills**: `preencher-empresa`, `preencher-voz`, `preencher-time`, `preencher-design` (estruturam texto livre em campos) + `sincronizar-meta` (operacional, puxa/grava dados Meta). `checar-marca`/`radar`/`copy-*` seguem Sonnet (julgamento). `atualizar` é determinístico (sem SDK). Commits: plugin `691ddf7`. **Propagado ao hosted** (`/var/lib/magnusos/plugin-skills/`, painel lê SKILL.md em runtime — sem rebuild). **PRÓXIMO: acumular custo Haiku no ledger via uso real (dogfood) → comparar com baseline Sonnet (`preencher-empresa`=$0.0667). ⚠️ Hipótese a validar: custo é dominado por cache-creation do contexto, não pelo modelo → Haiku pode render menos que enxugar prompt. O ledger dirá.**
