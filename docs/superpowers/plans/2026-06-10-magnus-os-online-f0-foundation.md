...
> **STATUS 2026-06-10 — ✅✅ TASK 8 + 9 LIVE (health server no ar): https://magnusos.yuribranco.com.br/api/health → `{"ok":true,"version":"f0"}`.** Provisionado no **VPS COMPARTILHADO do Portal** (`72.60.241.64`), NÃO dedicado. **Realidade da caixa divergiu do plano** (que assumia VPS dedicado + user `deploy` + bootstrap completo): node v22.22 + pm2 6.0.14 **já instalados**, pm2 roda **como root** (magnus+recasamento), `pm2-root` startup já `enabled`, ufw **inativo**. Então pulei o `bootstrap.sh` inteiro e fiz **deploy manual aditivo**: rsync dos 3 arquivos → `/var/www/magnusos-online`, `pm2 startOrReload` (root, junto dos outros) + `pm2 save`, nginx site aditivo (`nginx -t` gate passou, sites grupoaion/magnus/recasamento intactos), **certbot** emitiu cert (challenge passou pela Cloudflare proxied, igual aos outros). **Canary OK**: magnus 307→/dashboard (normal), recasamento 200, memória 1.3GB livre. magnusos no dump pm2 (reboot-safe). **`bootstrap.sh`/`deploy.yml` ficam como template do VPS DEDICADO da F1** (a F1 NÃO deve rodar no Portal — `clawdbot-gateway` já come 1.7GB; executor multi-tenant precisa de box próprio). CI/CD via GH Actions pendente de `gh auth refresh -h github.com -s workflow` (F0 usou deploy manual). **F0 COMPLETA.**
